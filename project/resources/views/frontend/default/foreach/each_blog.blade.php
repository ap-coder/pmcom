<div class="{{has_option('blog', 'post_column')}} col-md-6 col-sm-12">
    <div class="post-grid">
        <div class="post-grid-image">
            ...
            <img itemprop="image" src="{{get_attachment_url($item->thumbnail, 'medium')}}" alt="">
            <div class="post-meta clearfix">
                ...
                <ul class="float-right">  
                    ...
                    <li itemprop="about"><a href="{{url('blog/category/'.get_term_slug($item->term_id))}}" itemprop="url"><i class="pe-7s-ticket"></i> {{get_term_name($item->term_id)}}</a></li>
                </ul>
            </div>
        </div>
        <div class="post-detail">
            <h5 itemprop="text"><a href="{{url('post/'.$item->post_name)}}">{{$item->post_title}}</a></h5>
            <p itemprop="description">{{$item->post_excerpts}}</p>
            <a href="{{url('post/'.$item->post_name)}}" class="post-more" itemprop="url">{{lang('read_more')}}</a>
        </div>
    </div>
</div>